...
def define_client(api_name):

    ### define the client variable based on the model type

    ### inputs:
    # api_name: the service we are using to host the model (supported options: Azure, Bedrock, Fireworks) [str]
...
def obtain_acronym_defs(sentences, phrase, delimiters=['/', ' ', '-', '–', '@']):
